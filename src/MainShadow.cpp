...
const char* shadow_mapping_vertex = R"glsl(
#version 330 core
...
const char* shadow_mapping_fragment = R"glsl(
#version 330 core
out vec4 FragColor;
...
const char* shadow_mapping_vertex_depth = R"glsl(
	#version 330 core
layout (location = 0) in vec3 aPos;
...
const char* shadow_mapping_fragment_depth = R"glsl(
	#version 330 core
...
const char* debug_quad_vertex = R"glsl(
	#version 330 core
layout (location = 0) in vec3 aPos;
...
const char* debug_quad_fragment = R"glsl(
	#version 330 core
out vec4 FragColor;
...
int main()
{
    ...
    unsigned int woodTexture = loadTexture("../../../resources/wood.jpg");
...
void renderScene(const Shader& shader)
{
    ...
    renderCube();
    model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3((glfwGetTime() / 5.0f) + 2.0f, 0.0f, 1.0));
    model = glm::scale(model, glm::vec3(0.5f));
    shader.setMat4("model", model);
    renderCube();
    model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3((glfwGetTime() / 5.0f) + 2.5f, 0.0f, 2.0));
...
void processInput(GLFWwindow* window)
{
    ...
    keyController.processKeys(window);

    if (keyController.isKeyPressed(KeyInput::KeyDefinition::KEY_W)) {
        camera.UpdatePosition(Camera_Movement::FORWARD, 0.01f);
    }

    if (keyController.isKeyPressed(KeyInput::KeyDefinition::KEY_S)) {
        camera.UpdatePosition(Camera_Movement::BACKWARD, 0.01f);
    }
