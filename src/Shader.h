...
public:
    ...
    // ------------------------------------------------------------------------
    Shader(std::string vertexx, std::string fragmentt)
    {
        ...
        const char* vShaderCode = vertexx.c_str();
        const char* fShaderCode = fragmentt.c_str();
        // 2. compile shaders
    ...
    inline GLint getAttribLocation(const std::string& variableName, bool errorIfMissing = true) const {
        ...
        if (location == -1 && errorIfMissing) {
            std::cerr << "Error in vertex shader " << " : No variable : " << variableName << std::endl;
